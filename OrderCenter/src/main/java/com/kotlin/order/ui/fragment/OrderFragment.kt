...
class OrderFragment:BaseMvpFragment<OrderListPresenter>(), OrderListView {
                ...
                when(optType){
                    OrderConstant.OPT_ORDER_PAY -> {
                        ARouter.getInstance()
                                ...
                                .withInt(ProviderConstant.KEY_ORDER_ID, order.id) // 传入参数
                                .withLong(ProviderConstant.KEY_ORDER_PRICE, order.totalPrice)// 传入价格
                                .navigation()
                    }
    ...
    override fun onCanceOrderResult(result: Boolean) {
        toast("取消订单成功")
        loadData()
    }
}